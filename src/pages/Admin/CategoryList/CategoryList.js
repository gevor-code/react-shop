...
import {deleteCategory,getCateg} from "../../../store/user";
...
const CategoryList = () => {
    ...
    const getCategories = async () => {
        try {
            const response = await dispatch(getCateg());
            if (response) {
    ...
    const editCategory = async (categoryId) => {
        await navigate(`/admin/category-edit/${categoryId}`);
    };
                    ...
                    <table className="min-w-full text-center text-sm font-light bg-[#faebd7]">
                        ...
                        {categories.map((category) => (
                            <tr className="border-b dark:border-neutral-500" key={category.id}>
                                ...
                                <td className="border border-r-black  px-6 py-4 flex gap-6 justify-around ">
                                    ...
                                    <td className="border border-r flex justify-around">
